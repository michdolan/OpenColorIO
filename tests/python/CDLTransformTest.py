...
class CDLTransformTest(unittest.TestCase):
    ...
    TEST_CDL_SAT = 6.0
    TEST_CDL = """
<ColorCorrection id="abcd">
    <SOPNode>
        <Description>abcdefg</Description>
        <Slope>1.5 2 2.5</Slope>
        <Offset>3 3.5 4</Offset>
        <Power>4.5 5 5.5</Power>
    </SOPNode>
    <SatNode>
...
"""
    ...
    def test_id(self):
        ...
        for id in TEST_NAMES:
            self.cdl_tr.setID(id)
            self.assertEqual(id, self.cdl_tr.getID())
    ...
    def test_xml(self):
        ...
        self.cdl_tr.setXML(SIMPLE_CDL)
        self.assertEqual(self.cdl_tr.getID(), 'shot 042')
        self.assertEqual(self.cdl_tr.getDescription(),
                         'Cool look for forest scenes')
        self.assertListEqual(self.cdl_tr.getSlope(), [1.0, 2.0, 3.0])
        self.assertListEqual(self.cdl_tr.getOffset(), [4.0, 5.0, 6.0])
        self.assertListEqual(self.cdl_tr.getPower(), [7.0, 8.0, 9.0])
        self.assertEqual(self.cdl_tr.getSat(), 10.0)

        # Test the XML produced by this CDLTransform object.
        self.assertEqual(self.cdl_tr.getXML(), SIMPLE_CDL)
    ...
    def test_constructor_without_parameter(self):
        """
        Test CDLTransform constructor without parameters and validate its values.
        """
